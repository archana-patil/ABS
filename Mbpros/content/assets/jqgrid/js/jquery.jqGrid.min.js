...
"ltr"); if (b.p.rowList.length > 0) {
    g = "<td dir='" + i + "'>";
    //"Records Per Page added by Sunil Kumbhar"
    g = g + "<table><tr><td>Records Per Page : </td><td><select class='ui-pg-selbox' role='listbox'>"; for (j = 0; j < b.p.rowList.length; j++) g = g + ('<option role="option" value="' + b.p.rowList[j] + '"' + (b.p.rowNum === b.p.rowList[j] ? ' selected="selected"' : "") + ">" + b.p.rowList[j] + "</option>"); g = g + "</select></td></tr></table></td>"
} i === "rtl" && (f = f + g); b.p.pginput === true && (e = "<td dir='" + i + "'>" + a.jgrid.format(b.p.pgtext || "", "<input class='ui-pg-input' type='text' size='2' maxlength='7' value='0' role='textbox'/>", "<span id='sp_1_" +
...
l); h = a.fmatter.util.NumberFormat(h, l); j = a.fmatter.util.NumberFormat(j, l)
    ...
    //replace(" " , "") added by Sunil Kumbhar
    a(".ui-paging-info", m).html(a.jgrid.format(b.p.recordtext, i.replace(" ", ""), h.replace(" ", ""), j.replace(" ", "")))
